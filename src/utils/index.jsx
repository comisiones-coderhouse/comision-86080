...
import { collection, getDocs, addDoc, getDoc, doc/* getDoc, deleteDoc, , updateDoc */, query, where, /*  startAfter, startAt , endAt, endBefore , limit , orderBy  */ } from "firebase/firestore"
...
export async function fetchProductosAsync() {
    ...
    try {
        ...
        //console.log("🚀 ~ fetchProductosAsync ~ miConsulta:", miConsulta.docs)
        //console.log("🚀 ~ fetchProductosAsync ~ miConsulta:", miConsulta.docs[0])
        //console.log("🚀 ~ fetchProductosAsync ~ miConsulta:", miConsulta.docs[0].id)
        //console.log("🚀 ~ fetchProductosAsync ~ miConsulta:", miConsulta.docs[0].data())

        //5) Como firebase nos devuelve un array de referencias, tenemos que mapearlo para obtener los datos
        const usuariosFinales = miConsulta.docs.map((doc) => {
            return doc.data()
        })

        console.log("🚀 ~ fetchProductosAsync ~ usuariosFinales:", usuariosFinales)
...
export async function saveProduct(/* data */) {
    ...
    try {
        ...
        productos.forEach((producto) => {
            //4) La consulta
            //addDoc(unaCollection,data)
            const miConsulta = addDoc(productosCollection, producto)

            console.log("🚀 ~ saveProduct ~ miConsulta:", miConsulta)
        })
...
export async function getProductByID(id = 27) {
    try {

        const productosCollection = collection(db, "productos")

        const docRef = doc(productosCollection,"6aoViNuwQbSNEU2aNZ1g")

        const miConsulta = await getDoc(docRef)
        
        const elProducto = miConsulta.data()
        console.log("🚀 ~ getProductByID ~ elProducto:", elProducto)
        ...
        const productoFiltrado = miConsulta.docs.map((doc) => {
            return doc.data()
        })
